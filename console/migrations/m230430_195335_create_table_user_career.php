...
class m230430_195335_create_table_user_career extends Migration
{
    public function up()
    {
        $tableOptions = null;
        ...
        $this->createTable('{{%user_career}}', [
            ...
            'office' => $this->string()->Comment('Office'),
...
//            'year_end' => $this->date()->Comment('Year end'),
        ...
        $this->createIndex('country_id', '{{%user_career}}', 'country_id');
        $this->createIndex('city_id', '{{%user_career}}', 'city_id');
        $this->createIndex('user_id', '{{%user_career}}', 'user_id', true);
        $this->createIndex('region_id', '{{%user_career}}', 'region_id');
        $this->addForeignKey('user_career_ibfk_1', '{{%user_career}}', 'user_id', '{{%user}}', 'id', 'CASCADE', 'CASCADE');
        $this->addForeignKey('user_career_ibfk_2', '{{%user_career}}', 'country_id', '{{%country}}', 'id', 'RESTRICT', 'RESTRICT');
        $this->addForeignKey('user_career_ibfk_3', '{{%user_career}}', 'region_id', '{{%country_region}}', 'id', 'CASCADE', 'RESTRICT');
        $this->addForeignKey('user_career_ibfk_4', '{{%user_career}}', 'city_id', '{{%region_city}}', 'id', 'RESTRICT', 'RESTRICT');
    }
