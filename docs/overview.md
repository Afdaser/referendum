...
Механізм оновлення зображення капчі реалізовано через AJAX‑запит до `/site/captcha?refresh=1`, тож при кліку на картинку символи завжди змінюються.
Перевірка введеного коду тепер виконується коректно, а логін та email у формі реєстрації перевіряються вбудованим валідатором `unique`, що виключає хибні повідомлення про дублікати.

Файл подання модального вікна має назву `registrtion-step-one.php`, і всі згадки та маршрути узгоджені з цією назвою. Виджет капчі тепер використовує абсолютний шлях `/site/captcha`, завдяки чому зображення генерується коректно у всіх модулях.
