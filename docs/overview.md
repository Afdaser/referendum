...
Некритичні таблиці стилів (`outdatedBrowser.min.css`, `jquery.bxslider.css`, `font-awesome.css`) тепер завантажуються
асинхронно з атрибутами `media="print"` та `onload`. Це зменшує блокування відображення і пришвидшує початкове

завантаження сторінки. Для користувачів із вимкненим JavaScript додано `<noscript>`‑резервні посилання, а в атрибуті
`onload` передбачено очищення обробника після завантаження.
