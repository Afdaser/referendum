...
Сайт більше не виконує примусових 301-редіректів між мовними піддоменами. Будь-який домен чи піддомен вважається канонічним сам
для себе, тому canonical-посилання завжди формуються на поточний хост.

- головна сторінка (`/`) і її пагінації (`?page=2` тощо) віддають canonical з очищеним параметром `page=1`, щоб уникати дублів;
- сторінки опитувань (`/poll/{id}`) будують canonical на основі поточного домену без огляду на карту `langDomains`;
- сторінки тегів (`/tag/{tag}` і `/tag/{tag}/page/{n}`) так само зберігають хост, тож кожна мовна версія має власний canonical.

Такий підхід дозволяє основному домену й мовним піддоменам співіснувати без внутрішніх редіректів, але водночас підтримує
прозору логіку canonical-посилань для всіх типів сторінок.
