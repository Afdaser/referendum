...
- Форма реєстрації на фронтенді тепер використовує єдиний сценарій створення користувача через стандартну модель `SignupForm`.
- Перевірка даних виконується один раз, тож після успішної реєстрації користувач більше не бачить помилки про повторне введення логіна, пошти чи капчі.
- Після коректного заповнення форми сторінка оновлюється в поточному сценарії, а капча в модальному вікні перегенерується автоматично.
