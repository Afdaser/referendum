...
Таблиці стилів підключаються через атрибут `rel="preload"` і після завантаження автоматично стають звичайними `stylesheet`. Це прибирає блокувальні запити та пришвидшує відображення сторінки. Для користувачів із вимкненим JavaScript у `<head>` додається `<noscript>` із посиланнями на ті самі CSS‑файли, щоб верстка залишалася коректною.
