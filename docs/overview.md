...
Коментарі опитувань у мікророзмітці містять атрибут `datePublished` з датою у форматі `YYYY-MM-DD`. Значення записується в `content`, тож валідатор розпізнає дату незалежно від локалізованого тексту на сторінці.
