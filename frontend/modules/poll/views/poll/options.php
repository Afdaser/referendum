...
<?php if(!$poll->isShowResult()):?>
    <form method="post" action="<?= Url::toRoute(['/poll/vote']); ?>" class="poll-option-form">
        <!-- Відправляємо голос POST-запитом, щоб не показувати ідентифікатор опції в адресному рядку. -->
        <input type="hidden" name="<?= Yii::$app->request->csrfParam ?>" value="<?= Yii::$app->request->getCsrfToken() ?>">
        <!-- Одна форма для всіх варіантів, щоб зберегти вигляд без змін обгорток і екшен /poll/vote. -->
