...
<script>
	...
	$(function()
	{
		var maxLength = $('#answer_text').attr('maxlength');        //(1)
   ...
   $(document).on('click','.autocomplete-suggestion',function(){
    ...
    $(document).on('click','.pie_chart,.horizontal_b_chart,.vertical_b_chart',function(){
        $('#container<?php echo $poll->id; ?>').removeClass('pie').removeClass('bar').removeClass('column');
        $('#container<?php echo $poll->id; ?>').addClass($(this).attr('data-id'));
        filterDataChart(<?php echo $poll->id;?>,'<?php echo $poll->title;?>');
    });
</script>